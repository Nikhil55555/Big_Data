...
object UserTimeSpent {
def main ( args : Array[String]) :Unit ={
val conf =new SparkConf().setAppName("userTimeSpent");
...
webdf.createOrReplaceTempView("webclickView") 
val webresult = sqlContext.sql("select customer_ip,sum(timespent) from webclickView group by customer_ip order by sum(timespent) desc ")
